...
### Data for Drayman component events

Component events are simple props which name starts with an `on` prefix. Because components and props (events) are created by you, you can pass any data you want. You can read [here](./helpers/props) how to work with component props.
...
- debounce - `number` or `object`:
  - wait - `number` - number of ms to delay;
  - trailing - `boolean` - invokes function on the trailing edge;
  - leading - `boolean` - invokes function on the leading edge.
